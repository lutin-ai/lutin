...
pub fn BoundedString(comptime capacity: usize) type {
    return struct {
        inner: std.BoundedArray(u8, capacity) = .{},
        ...
        pub fn init_with_contents(content: []const u8) !Self {
            var s = Self{
                .inner = .{},
            };
            try s.inner.appendSlice(content);
            return s;
        }

        pub fn init_format(comptime fmt: []const u8, args: anytype) !Self {
            var s = Self{};
            try std.fmt.format(s.inner.writer(), fmt, args);
            return s;
        }
    };
}
...
pub const String = struct {
    ...
    pub fn init_owned(allocator: std.mem.Allocator, owned_str: []u8) String {
        // for windows non-ascii characters
        // check if the system is windows
        if (builtin.os.tag == std.Target.Os.Tag.windows) {
            _ = std.os.windows.kernel32.SetConsoleOutputCP(65001);
    ...
    pub fn to_owned(self: String) Error!?[]u8 {
        if (self.buffer != null) {
    ...
    pub fn char_at(self: String, index: usize) ?[]const u8 {
        if (self.buffer) |buffer| {
    ...
    pub fn remove_range(self: *String, start: usize, end: usize) Error!void {
    ...
    pub fn trim_start(self: *String, whitelist: []const u8) void {
        if (self.buffer) |buffer| {
            var i: usize = 0;
            while (i < self.size) : (i += 1) {
                const size = String.getUTF8Size(buffer[i]);
    ...
    pub fn trim_end(self: *String, whitelist: []const u8) void {
        self.reverse();
    ...
    pub inline fn is_empty(self: String) bool {
        return self.size == 0;
    }
    ...
    pub fn split_all(self: *const String, delimiters: []const u8) ![][]const u8 {
    ...
    pub fn split_to_string(self: *const String, delimiters: []const u8, index: usize) Error!?String {
    ...
    pub fn split_all_to_strings(self: *const String, delimiters: []const u8) ![]String {
    ...
    pub fn to_lowercase(self: *String) void {
        if (self.buffer) |buffer| {
            var i: usize = 0;
            while (i < self.size) {
                const size = String.getUTF8Size(buffer[i]);
    ...
    /// Converts all (ASCII) uppercase letters to lowercase
    pub fn to_uppercase(self: *String) void {
        if (self.buffer) |buffer| {
            var i: usize = 0;
            while (i < self.size) {
                const size = String.getUTF8Size(buffer[i]);
    ...
    pub fn to_capitalized(self: *String) void {
    ...
    inline fn is_UTF8_byte(byte: u8) bool {
        return ((byte & 0x80) > 0) and (((byte << 1) & 0x80) == 0);
    }

    /// Returns the real index of a unicode string literal
    fn get_index(unicode: []const u8, index: usize, real: bool) ?usize {
        var i: usize = 0;
        var j: usize = 0;
        while (i < unicode.len) {
            if (real) {
    ...
    inline fn get_UTF8_size(char: u8) u3 {
    ...
    pub fn includes_string(self: *String, needle: String) bool {
    ...
    pub fn includes_literal(self: *String, needle: []const u8) bool {
