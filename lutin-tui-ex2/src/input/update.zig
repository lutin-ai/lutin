...
fn apply(m: *Model, act: Action, now_ms: i64, ctx: ?*app.Ctx) !void {
    switch (act) {
        ...
        .split_toggle => {
            ...
            if (tab.split_percent == 0) {
                tab.split_percent = 35; // default 35/65 split
            } else {
                tab.split_percent = 0;
            }
            m.leader = .none;
        ...
        .split_cycle_orientation => {
            const tab = m.activeTab();
            if (tab.split_percent == 0) {
                tab.split_percent = 40;
                tab.split_side = .lhs;
            } else {
                // cycle: vertical -> horizontal -> none
                if (tab.split_side == .lhs or tab.split_side == .rhs) {
                    tab.split_side = .top;
                } else {
                    tab.split_percent = 0;
                    tab.split_side = .lhs;
        ...
        .split_resize_left => resizeSplit(m, -1),
        .split_resize_right => resizeSplit(m, 1),
        .split_resize_up => resizeSplit(m, -1),
        .split_resize_down => resizeSplit(m, 1),
        .next_tab => {
...
fn resizeSplit(m: *Model, dir: i8) void {
    const t = m.activeTab();
    if (t.split_percent == 0) {
        // No split active, start at default
        t.split_percent = if (dir < 0) 30 else 40;
    } else {
        const delta: i8 = dir * 5;
        t.split_percent = @intCast(@max(@as(u8, 10), @min(@as(u8, 90), t.split_percent + @as(u8, @intCast(delta)))));
    }
    m.dirty = true;
}
