...
pub const SplitMode = enum { none, vertical, horizontal };

pub const SplitPosition = enum { lhs, rhs, top, bottom };
...
pub const Tab = struct {
    ...
    /// Horizontal split position as percentage (1-99). 0 means no split.
    split_percent: u8 = 0,
    /// Which side to constrain when resizing (for vertical split) or
    /// which edge to drag from (for horizontal). Controls growth direction.
    split_side: SplitPosition = .lhs,
