...
pub fn renderStatus(win: Window, m: *const Model, now_ms: i64, th: theme.Theme) void {
    ...
    if (tab.split_percent > 0) {
        const orient = switch (tab.split_side) {
            .lhs, .rhs => "⫝",
            .top, .bottom => "⫞",
        };
        col += writeSeg(win, col, 0, " ", th.style(.chrome));
        col += writeSeg(win, col, 0, orient, th.style(.accent));
        var pct_buf: [4]u8 = undefined;
        const pct_str = std.fmt.bufPrint(&pct_buf, "{}%", .{tab.split_percent}) catch "%";
        col += writeSeg(win, col, 0, pct_str, th.style(.chrome_dim));
    }
