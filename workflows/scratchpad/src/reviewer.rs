...
use crate::types::{ContextItem, Plan, Principle, Verdict};
...
pub async fn review_principle(
    ...
    let user = build_user_prompt(principle, plan, history);
...
fn build_user_prompt(principle: &Principle, plan: &Plan, history: &[Message]) -> String {
    let mut s = String::new();

    s.push_str("Plan context:\n");
    s.push_str(&format!("  goal: {}\n", plan.goal));
    // s.push_str(&format!("  why_this_tool: {}\n", plan.why_this_tool));
    // if !plan.considerations.is_empty() {
    //     s.push_str("  considerations:\n");
    ...
    let args = serde_json::to_string(&plan.args).unwrap_or_else(|_| "<unserializable>".into());
    s.push_str(&format!("  arguments: {args}\n"));

    if principle.context.contains(&ContextItem::Chat) && !history.is_empty() {
        s.push_str("\nConversation history before this step:\n");
        for m in history {
