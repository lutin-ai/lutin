//! WebSocket connection handler. Replaces the engine binary's
//! accept-and-drop scaffold with a real hello → auth → dispatch loop.
//!
//! Runtime side is still stubbed — `SendMessage` returns a clear
//! `Internal` error until the agent runner is wired in step 2.
...
use crate::wire::{
    ChatError, ChatEvent, ChatOk, ChatRequest, ChatResponse, PersonaInfo, SessionState, StepId,
    StepStatus, Turn, TurnId, WirePlan, WireStep,
    decode as wire_decode, encode as wire_encode,
};
...
fn project_turns(messages: &[lutin_llm::Message], steps: &[StepRecord]) -> Vec<Turn> {
    ...
    for msg in messages {
        match msg {
            ...
            lutin_llm::Message::Assistant { tool_calls, .. } if !tool_calls.is_empty() => {
                if let Some(record) = steps.get(step_idx) {
                    turns.push(Turn::Step(WireStep {
                        id: StepId(step_idx as u64),
                        status: StepStatus::Done {
                            plan: WirePlan::from_plan(&record.plan),
    ...
    match req {
        ChatRequest::Subscribe => {
            ...
            let turns = match store::load(&state.state_dir) {
                Ok(Some(saved)) => project_turns(&saved.messages, &saved.steps),
                Ok(None) => Vec::new(),
                Err(e) => {
