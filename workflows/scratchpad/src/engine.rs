//! Scratchpad workflow engine binary.
//!
//! One subprocess per session. Spawned by `lutin-project` with the same
//! env handoff convention as `principled`. Binds a loopback TCP
//! listener, publishes its bound addr to `LUTIN_WORKFLOW_HANDOFF_PATH`,
//! then serves WebSocket connections protected by `WorkflowSession`-
//! scoped tokens.
...
use scratchpad::runner::{RunnerCtx, run_agent_loop};
use scratchpad::serve::{AppState, serve_conn};
...
async fn main() -> Result<()> {
    ...
    info!(%bound, session = %env.session, "scratchpad workflow listening");
