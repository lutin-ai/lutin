...
export function App({ lutin }: Props) {
  ...
  useEffect(() => {
    ...
    // Replay the persisted reviewer log so the sidebar shows pre-
    // subscribe activity. Live updates ride the `reviewerCompleted`
    // broadcast and append onto this snapshot through the reducer.
    lutin
      .request(encodeChatRequest({ kind: "listReviews" }))
      .then((body) => {
        if (cancelled) return;
        try {
          dispatch({ type: "response", response: decodeChatResponse(body) });
        } catch (err) {
          console.warn("malformed ListReviews response", err);
        }
      })
      .catch(() => {
        // Sidebar is decorative — failure leaves the log empty.
      });
  ...
  const dispatchSend = useCallback(
    (text: string) => {
      ...
      // reloads persona TOML on every turn, so the chip would otherwise
      // drift from what the LLM will use.
      refreshPersonas();
      lutin
...
// Inline styles keep the principled-specific shell out of the shared
// chat-widgets theme. The sidebar has its own fixed width and the
// chat pane fills the remainder; both share the parent's height.
